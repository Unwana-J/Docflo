...
import { suggestFieldMapping } from '../services/geminiService';
...
const BulkGenerator: React.FC<BulkGeneratorProps> = ({ activeTeam }) => {
  ...
  const autoMapHeaders = async (csvHeaders: string[], template: DocumentTemplate) => {
    ...
    try {
      ...
      const aiMapping = await suggestFieldMapping(fieldList, headerList);
        ...
        {[
          ...
          return (
            <React.Fragment key={s.id}>
              <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border-2 transition-all ${isCurrent ? 'bg-blue-600 border-blue-600 text-white shadow-xl scale-105' :
